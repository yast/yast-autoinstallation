...
module Yast
  class AutoinstPartPlanClass < Module
	...
	ok
      ...
      drives = Builtins.maplist(_StorageMap) do |k, v|
        ...
        Builtins.foreach(Ops.get_list(v, "partitions", [])) do |pe|
          ...
          new_pe["ignore_fstab"] = pe["ignore_fstab"] if pe["ignore_fstab"]
          skipwin = false
    ...
    def Import(settings)
      ...
      tmpfs_device = settings.find { |device| device["type"] == :CT_TMPFS }
      if tmpfs_device && tmpfs_device.has_key?("partitions")
        tmpfs_device["partitions"].delete_if { |partition| partition["ignore_fstab"] }
      end
